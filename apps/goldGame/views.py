...
def process(request):
    ...
    if request.method == "POST":
        ...
        haha = request.session['journal']

        if(randomNum < 0):
            haha.append("Entered a casino and lost {} gold...Ouch. {}".format(-randomNum, time))
    
        else :
            haha.append("Earned {} gold from the {}! {}".format(randomNum, building, time))
    
    request.session['journal'] = haha
